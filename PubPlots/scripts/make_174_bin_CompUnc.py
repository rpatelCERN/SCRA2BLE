...
def make_174_bin_CompUnc(plot_title, fileNominal, fileComp,BkgName, TF,TFErr):
    doPull=False;
    ...
    #fZinv2018=TFile("../DatacardBuilder/inputHistograms/histograms_137.4fb/ZinvHistos.root")
    ...
    hbg_pred = f_bkg1.Get("hCV").Clone("hbg_pred")
    hbg_pred.Reset()
    hbg_pred.SetTitle("")
    ...
    hbg_pred.SetMaximum(1.5)
    hbg_pred.SetMinimum(0.1)

    #data_obs = DataObs(lostlept)
    #ratio = ExpExpRatio(sumBG, sumBG) # note that this also sets the style
    ratio = ObsExpRatio(data_obs, sumBG) # note that this also sets the style
    ...
    Base2016InputErr=fZinv2016.Get(TFErr)
    ...
    leg1.AddEntry(Base2016Input, "2016 %s " %BkgName, "l")
   ...
   # leg2.AddEntry(hlostlept, "#splitline{Lost}{lepton}", "f")
    leg2.AddEntry(TestNewInput, "Run2 Input: New binning", "l")
    #leg1.Draw()
    leg1.Draw()
	...
	hratdummy.GetYaxis().SetTitle("Rel. Diff");
        ...
        ey_h = []
	y0=[]
        nbins = Base2016Input.GetNbinsX()
        for ibin in range(nbins):
            x.append(Base2016Input.GetBinCenter(ibin+1))
            ex_l.append(0.5)
            ex_h.append(0.5)
	    y0.append(0.0)
            if TestNewInput.GetBinContent(ibin+1)>0 and Base2016Input.GetBinContent(ibin+1)>0:                
                #y.append( (Base2016Input.GetBinContent(ibin+1)-TestNewInput.GetBinContent(ibin+1)))
                ey_h.append( abs( 1.0-Base2016InputErr.GetBinContent(ibin+1)) / Base2016Input.GetBinContent(ibin+1))
                ey_l.append( abs(1.0- Base2016InputErr.GetBinContent(ibin+1) )/ Base2016Input.GetBinContent(ibin+1))
        ...
        gMarkers2 = TGraphAsymmErrors(nbins, array('d', x), array('d', y0),array('d',ex_l), array('d',ex_h),array('d',ey_l), array('d',ey_h))
        gMarkers = TGraph(nbins, array('d', x), array('d', y))
        gMarkers2.SetFillColor(2029)
