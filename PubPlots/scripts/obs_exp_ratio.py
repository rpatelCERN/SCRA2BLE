...
class ObsExpRatio:
    ...
    def GetPullDist(self): # this will just be a TH1 of markers with no error bars
        ...
        for ibin in range(hpull.GetNbinsX()):
            ...
            else:pull = (self.data_obs.hist.GetBinContent(ibin+1)-self.bg_pred.hCV.GetBinContent(ibin+1))/sqrt(self.bg_pred.gFull.GetErrorYlow(ibin)**2 + self.bg_pred.hCV.GetBinContent(ibin+1))
