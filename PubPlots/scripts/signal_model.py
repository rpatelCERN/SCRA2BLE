...
class SignalModel:
    ...
    def AggregateBins(self, agg_bins): # creates a histogram from aggregated bins -- this is used in fill_signal_hists.py
        hname = self.inhist.GetName()
        hagg = TH1D(hname, "", len(agg_bins), 0.5, float(len(agg_bins))+0.5)
