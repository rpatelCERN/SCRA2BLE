...
def fill_postfit(inputfile = 'inputs/bg_hists/fitDiagnosticstestCards-allBkgs-T1tttt_2000_200-41.5-mu0.0.root', outputfile = 'postfit_hists.root', nbins = 174):
    ...
    for name, asrs in asr_sets.items():
       ...
       hCV_QCDASR = Uncertainty(hQCDCV, "all").AggregateBins(asrs, asr_xtitle[name], asr_xbins[name]).hist # pretending the CV is a fully-correlated uncertainty b/c we need to add it linearly
       hCV_ZASR = Uncertainty(hZCV, "all").AggregateBins(asrs, asr_xtitle[name], asr_xbins[name]).hist # pretending the CV is a fully-correlated uncertainty b/c we need to add it linearly
       hCV_WTopASR = Uncertainty(hWTopCV, "all").AggregateBins(asrs, asr_xtitle[name], asr_xbins[name]).hist # pretending the CV is a fully-correlated uncertainty b/c we need to add it linearly
