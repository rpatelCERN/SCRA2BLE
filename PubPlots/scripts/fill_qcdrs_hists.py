...
def fill_qcdrs_hists(inputfile = 'inputs/bg_hists/QcdPredictionRandS_35.9.root', outputfile = 'qcdrs_hists.root', nbins = 174, lumiSF = 1.):
   ...
   for h in infile.GetListOfKeys():
       ...
       if h.GetName().find('CV') >= 0 or h.GetName().find('Stat') >= 0 or h.GetName().find('hLowMhtBaseline_')>=0 or h.GetName().find('hBaseline_SearchBinsRplusS')>=0:
           continue
       print(h.GetName())
       # convert to absolute
       hout = h.Clone()
	   ...
	   if h.GetName().find("PredictionBTag")>=0:
		if h.GetBinContent(ibin)<0.0001:h.SetBinContent(ibin,1.0);
