...
parser.add_option("-o", "--output", dest="outfile", default=None, type="string", help="")
    ...
    for fit_name, nuis_x in [('b', nuis_b), ('s',nuis_s)]:
        if nuis_x == None:
		  ...
		  hist_prefit.SetBinError(nuis_p_i,sigma_p)
                ...
                if sigma_p>0:valShift = (nuis_x.getVal() - mean_p)/sigma_p
                if fit_name == 'b':
                    pulls.append(valShift)
                if sigma_p>0:sigShift = nuis_x.getError()/sigma_p
                if options.abs:
...
if options.format == 'text':
	...
	fout.write(fmtstring %('name', 'b-only fit', 's+b fit', 'rho'))
	fout.write('\n')
...
for n in names:
    ...
    if options.abs:
       ...
       fout.write(fmtstring %(n, v[0], v[1], v[2]))
       fout.write('\n')
