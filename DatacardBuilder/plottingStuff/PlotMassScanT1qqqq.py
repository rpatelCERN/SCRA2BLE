...
flist=open("listofFilesT1qqqq.txt", 'r')
...
for m in range(len(mGo)):
	filein=TFile("results_T1qqqq_%d_%d.root" %(int(mGo[m]), int(mLsp[m])))
	...
	if mGo[m]<1300 and mLsp[m]<400: 
		...
		continue
	if not (mGo[m]<1300 and mLsp[m]<400) and ((mGo[m]<1500)):
	     if (mLsp[m]<=400 and mGo[m]>=1300) or mGo[m]-mLsp[m]>=250:
		 for i in range(0,2):
                        for j in range(0,2):
                                h2_MassScan.Fill(mGo[m]+(i*25)+0.5,mLsp[m]+(j*25)+0.5,t.limit_exp)
                                h2_MassScanObs.Fill(mGo[m]+(i*25)+0.5,mLsp[m]+(j*25)+0.5, t.limit_obs)
		...
		h2_MassScan.Fill(mGo[m]+(0.5),mLsp[m]+(0.5),t.limit_exp)
	     ...
	     continue
	if mLsp[m]>=1200 or (mLsp[m]<=400 and mGo[m]>=1300):
		for i in range(0,2):
			for j in range(0,2):
				h2_MassScanXsec.Fill(mGo[m]+(i*25)+0.5,mLsp[m]+(j*25)+0.5,ExpUL)
				h2_MassScanObsXsec.Fill(mGo[m]+(i*25)+0.5,mLsp[m]+(j*25)+0.5, ObsUL)
...
fout=TFile("testT1qqqq.root", 'recreate')
