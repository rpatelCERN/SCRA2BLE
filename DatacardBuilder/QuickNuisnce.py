...
for q in QCDSystematics:
	...
	NuisanceName="%s" %(q)
	cmmd="combine -M MultiDimFit --rMin=-5 --rMax=5  -t -1 --expectSignal 0 --robustFit 1 -n _paramFit_Test_%s --algo impact --redefineSignalPOIs r -P %s --floatOtherPOIs 1 --saveInactivePOI 1 -m 125 -d %s/allcards.root  "%(NuisanceName,NuisanceName,sys.argv[1])
...
for j in range(0,5):
	for mht in MHTHTLabels:
		for b in range(0,4):
			...
			cmmd="combine -M MultiDimFit --robustFit 1 -t -1 --expectSignal 0  -n _paramFit_Test_%s --rMin=-50 --rMax=50   --algo impact --redefineSignalPOIs r  -P %s --floatOtherPOIs 1 --saveInactivePOI 1 -m 125 -d %s/allcards.root "%(NuisanceName,NuisanceName,sys.argv[1])
			os.system("Qsub -l lnxfarm -o OutPut_LogFile%s -e %s " %(NuisanceName,cmmd))
for l in LLSystematicsList:
	NuisanceName="%s" %(l)
	cmmd="combine -M MultiDimFit --rMin=-10 --rMax=10  --robustFit 1 -n _paramFit_Test_%s --algo impact --redefineSignalPOIs r -P %s --floatOtherPOIs 1 --saveInactivePOI 1 -m 125 -t -1  --expectSignal 0  -d %s/allcards.root  "%(NuisanceName,NuisanceName,sys.argv[1])
	os.system("Qsub -l lnxfarm -o OutPut_LogFile%s -e %s " %(NuisanceName,cmmd))
	print cmmd
		...
		for b in range(0,4):
			for n in LostLepUncorrel:
				NuisanceName="%s_NJets%d_BTags%d_%s" %(n,j, b,mht)
                        	cmmd="combine -M MultiDimFit --robustFit 1 -t -1 --expectSignal 0  -n _paramFit_Test_%s --rMin=-10 --rMax=10 --algo impact --redefineSignalPOIs r -P %s --floatOtherPOIs 1 --saveInactivePOI 1 -m 125 -d %s/allcards.root "%(NuisanceName,NuisanceName,sys.argv[1])
				os.system("Qsub -l lnxfarm -o OutPut_LogFile%s -e %s " %(NuisanceName,cmmd))
