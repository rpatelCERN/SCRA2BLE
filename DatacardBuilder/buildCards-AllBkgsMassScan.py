...
if __name__ == '__main__':
	...
	model=options.signal+options.mGo
	tag = options.tag;
	lumi = float(options.lumi);
	signalmu = float(options.mu);
	odir = 'testCards-%s-%s-%s-mu%0.1f/' % ( tag,model,str(round(lumi,1)), signalmu );
	...
	signalSFB_file =TFile("inputHistograms/histograms_1p3fb/fastsimSignalScan/"+"/RA2bin_signal.root");
	signalSysSFUp_file=TFile("inputHistograms/histograms_1p3fb/fastsimSignalScan/RA2bin_signal_btagCFuncUp.root");
	signalSysSFDown_file=TFile("inputHistograms/histograms_1p3fb/fastsimSignalScan/RA2bin_signal_btagCFuncDown.root");
	signalSysMisSFUp_file=TFile("inputHistograms/histograms_1p3fb/fastsimSignalScan/RA2bin_signal_mistagCFuncUp.root");
	signalSysMisSFDown_file=TFile("inputHistograms/histograms_1p3fb/fastsimSignalScan/RA2bin_signal_mistagCFuncDown.root");
	sphotonRegion_file = TFile("inputHistograms/histograms_225fb/"+"/RA2bin_GJet_CleanVars.root");#Just need this for bin labels
	#lowdphiRegion_file = TFile(idir+"/RA2bin_LDP.root");
		
	# signal --------
	signalRegion_sigHist = signalSFB_file.Get("RA2bin_"+"T1bbbb_"+options.mGo+"_"+options.mLSP+"_fast");
	signalRegion_sigHistSFUp= signalSysSFUp_file.Get("RA2bin_"+"T1bbbb_"+options.mGo+"_"+options.mLSP+"_fast");
	signalRegion_sigHistSFDown= signalSysSFDown_file.Get("RA2bin_"+"T1bbbb_"+options.mGo+"_"+options.mLSP+"_fast");
							
	signalRegion_sigHistMisSFUp = signalSysMisSFUp_file.Get("RA2bin_"+"T1bbbb_"+options.mGo+"_"+options.mLSP+"_fast")
	signalRegion_sigHistMisSFDown = signalSysMisSFDown_file.Get("RA2bin_"+"T1bbbb_"+options.mGo+"_"+options.mLSP+"_fast")
...
#	signalRegion_tauList = textToList( "inputsHadTau/HadTauMCPred%sfb.txt" % (str(int(lumi))), 0 );
	...
	phoRegion_sigHist = sphotonRegion_file.Get("RA2bin_SMSbbbb1000")
   ...
   #compute the Zvv actual yields to be put in the observed lines
	...
	pdf=1.3
	ISR=1.01
	if(model=='SMSqqqq1400' or model=='SMStttt1200' or model=='SMSbbbb1000'):
		ISR=1.08
		pdf=1.10
